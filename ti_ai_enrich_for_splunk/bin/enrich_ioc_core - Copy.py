...
def slim_otx(resp):
    ...
    return {
        ...
        "type": resp.get("type"),
        "pulse_count": len(resp.get("pulses", []) or []),
        "tags": [t for p in (resp.get("pulses") or []) for t in (p.get("tags") or [])],
        "passive_dns_count": len(resp.get("passive_dns", []) or []),
        "related_count": len(resp.get("related", []) or []),
        "malware_families": [m.get("family") for m in (resp.get("malware_families") or []) if m.get("family")]
    }
...
class EnrichEngine:
        ...
        with ThreadPoolExecutor() as pool:
            ...
            for name, fut in futures.items():
                try:
                    ...
                    if name == "otx": sources[name] = slim_otx(raw)
                    elif name == "abuseipdb": sources[name] = slim_abuseipdb(raw)
                    elif name == "greynoise": sources[name] = slim_greynoise(raw)
                    elif name == "urlhaus": sources[name] = slim_urlhaus(raw)
                    else: sources[name] = raw
    ...
    def _derive_fields(self, ioc_type, ioc_value, sources, base_tags):
        ...
        tags = merge_unique_tags(tags, otx_tags)
        ...
        if isinstance(abuse_cats, list) and abuse_cats:
            ...
            tags = merge_unique_tags(tags, pretty)
        else:
